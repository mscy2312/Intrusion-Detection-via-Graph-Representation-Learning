...
# -*- coding: utf-8 -*-
r"""
make_multi_windows_tensors_parallel.py (FIXED)
...
ALL_EVENT_TYPES = [
    ...
    "EVENT_LOGCLEAR","EVENT_LOGIN","EVENT_LOGOUT","EVENT_LSEEK","EVENT_MMAP","EVENT_MODIFY_FILE_ATTRIBUTES",
    "EVENT_MODIFY_PROCESS","EVENT_MOUNT","EVENT_MPROTECT","EVENT_OPEN","EVENT_OTHER","EVENT_READ",
    "EVENT_READ_SOCKET_PARAMS","EVENT_RECVFROM","EVENT_RECVMSG","EVENT_RENAME","EVENT_SENDTO",
    "EVENT_SENDMSG","EVENT_SERVICEINSTALL","EVENT_SHM","EVENT_SIGNAL","EVENT_STARTSERVICE",
    "EVENT_TRUNCATE","EVENT_UMOUNT","EVENT_UNIT","EVENT_UNLINK","EVENT_UPDATE","EVENT_WAIT",
    "EVENT_WRITE","EVENT_WRITE_SOCKET_PARAMS","EVENT_TEE","EVENT_SPLICE","EVENT_VMSPLICE",
    "EVENT_INIT_MODULE","EVENT_FINIT_MODULE"
]

# --- FIXED NODE TYPES (global) ---
NODE_TYPES = ["subjects", "fileobjects", "netflows", "memory", "ipc", "registrykeys", "unknown"]
...
def node_types_from_parquet(base: Path, node_uuids:set) -> Dict[str,str]:
    if not node_uuids:
        return {}
    uuid2type={u:"unknown" for u in node_uuids}
    ...
    for folder in ["subjects","fileobjects","netflows"]:
        ...
        for r in table.to_pylist():
            ...
            if u in remaining:
                uuid2type[u]=folder
        remaining = {u for u in remaining if uuid2type[u]=="unknown"}
    return uuid2type

# ---------------------- tensors ------------------------
def build_tensors(edges, node_types_map, min_ts, max_ts, all_event_types):
    """
    Builds tensors for a single window graph.
    Node features = one-hot over FIXED NODE_TYPES + [deg_in, deg_out, deg_total]
    Edge features = one-hot over FIXED ALL_EVENT_TYPES + [norm_timestamp]
    """
    nodelist = sorted(node_types_map.keys())
    nid = {u: i for i, u in enumerate(nodelist)}

    # --- FIXED EVENT TYPES ---
    etypes = list(all_event_types)
    event_type2idx = {t: i for i, t in enumerate(etypes)}
    num_et = len(etypes)

    # --- FIXED NODE TYPES ---
    node_type2idx = {t: i for i, t in enumerate(NODE_TYPES)}
    num_nt = len(NODE_TYPES)

    # degrees
    deg_in = {u: 0 for u in nodelist}
    deg_out = {u: 0 for u in nodelist}
    ...
    for u in nodelist:
        ...
        t_idx = node_type2idx.get(t, node_type2idx["unknown"])
        onehot = [0.0] * num_nt
        onehot[t_idx] = 1.0
        fin = deg_in[u] / max_in
        fout = deg_out[u] / max_out
        ftot = deg_tot[u] / max_tot
    ...
    meta = {
        "node_index_map": {u: nid[u] for u in nodelist},
...
# ---------------- worker (unchanged logic, just calls build_tensors) ------------
def process_file_worker(file_path: str, base_dir: str, out_dir: str,
    gt_ivs: List[Tuple[int,int]], win_ns: int, stride_ns: int,
    max_windows: int, need_label: int):
    ...
    for (ws,we,edges,node_uuids,min_ts,max_ts) in collect_successive_windows_from_file(
        ...
        x, edge_index, edge_attr, meta = build_tensors(edges, uuid_map, min_ts, max_ts, ALL_EVENT_TYPES)
        ...
        out_rows.append({
            "graph_file": name,"start_ns": ws,"end_ns": we,"label": label,
            "num_nodes": int(x.shape[0]),"num_edges": int(edge_index.shape[1]),
            "file_source": file.name,
            "read_parse_sec": round((t0 - t_file_start), 4),
...
# ---------------- collector (unchanged) ----------------
def collect_successive_windows_from_file(file: Path, win_ns: int, stride_ns: int, edges_cap: Optional[int], max_windows: int):
    pf = pq.ParquetFile(file)
    schema = set(pf.schema_arrow.names)
    cols = [c for c in EVENT_COLS if c in schema]
    if not {"timestampNanos","subject_uuid"}.issubset(set(cols)): return
    ...
    for batch in pf.iter_batches(batch_size=BATCH_SIZE, columns=cols):
        ...
        get = tbl.schema.get_field_index
        ...
        for i, ts in enumerate(ts_col):
            ...
            while ts >= current_we:
                out = flush_current()
                if out and out[2]:
                    yield out; windows_yielded += 1
                    if windows_yielded >= max_windows: return
            ...
            if u and v1:
                ...
                if cap and len(edges)>=cap:
                    out = flush_current()
                    if out and out[2]: yield out; windows_yielded += 1
                    if windows_yielded >= max_windows: return
                    current_ws += stride_ns; current_we = current_ws + win_ns
            if u and v2:
                ...
                if cap and len(edges)>=cap:
                    ...
                    current_ws += stride_ns; current_we = current_ws + win_ns
    out = flush_current()
    if out and out[2]: yield out

# ------------------------------ main -------------------
def main():
    out_dir = OUTPUT_DIR; out_dir.mkdir(parents=True, exist_ok=True)
    ...
    if SHUFFLE_FILES: random.shuffle(files)
    ...
    win_ns = WINDOW_MIN * NS_PER_MIN; stride_ns = STRIDE_MIN * NS_PER_MIN
    ...
    index_wr = csv.writer(index_csv); prof_wr  = csv.writer(prof_csv)
    ...
    written_mal=written_ben=sum_nodes=sum_edges=0; t0 = time.perf_counter()
    futures = []
    with ProcessPoolExecutor(max_workers=MAX_WORKERS) as ex:
        for f in mal_submit:
            futures.append(ex.submit(process_file_worker,str(f),str(INPUT_DIR),str(out_dir),gt_ivs,win_ns,stride_ns,MAL_WINS_PER_FILE,1))
        for f in ben_submit:
            futures.append(ex.submit(process_file_worker,str(f),str(INPUT_DIR),str(out_dir),gt_ivs,win_ns,stride_ns,BEN_WINS_PER_FILE,0))
        pbar = tqdm(total=(TARGET_MAL + TARGET_BENIGN), desc="Windows", unit="win")
        for fut in as_completed(futures):
            try: rows = fut.result() or []
            except Exception as e: log.warning(f"Worker failed: {e}"); rows=[]
            for r in rows:
                if r["label"]==1 and written_mal>=TARGET_MAL: continue
                if r["label"]==0 and written_ben>=TARGET_BENIGN: continue
                index_wr.writerow([r["graph_file"],r["start_ns"],r["end_ns"],r["label"],r["num_nodes"],r["num_edges"],r["file_source"]])
                prof_wr.writerow([r["graph_file"],r["read_parse_sec"],r["uuid_typing_sec"],r["tensor_build_sec"],r["save_sec"],r["total_window_sec"]])
                sum_nodes+=r["num_nodes"]; sum_edges+=r["num_edges"]
                if r["label"]==1: written_mal+=1
                else: written_ben+=1
                pbar.update(1)
                if written_mal>=TARGET_MAL and written_ben>=TARGET_BENIGN: break
            if written_mal>=TARGET_MAL and written_ben>=TARGET_BENIGN: break
        pbar.close()
    index_csv.close(); prof_csv.close()
    total_elapsed = time.perf_counter()-t0; total_written=written_mal+written_ben
